...
from functionality.fishing_actions import repairing, select_bait, fish_notice, reel_fish, pause, cast
...
def fishing_loop():
    ...
    if gv.last_results.is_full_of("0"):
        if config_dict["repairing"]["enable"].get() == 1:
            should_repair_in = -1 * (int(time()) - gv.last_repair_time - config_dict["repairing"]["every"].get())
            debug("Repair in: " + str(should_repair_in))
            if should_repair_in < 0:
                gv.last_repair_time = int(time())
                info("Repairing")
                repairing()
                if config_dict["bait"]["enable"].get():
                    info("Selecting bait")
                    select_bait()
...
def call_appropriate_fishing_action():
    result_from_model = image_recognition_result(
        config_dict["fishing"]["x"].get(),
        config_dict["fishing"]["y"].get(),
        config_dict["fishing"]["width"].get(),
        config_dict["fishing"]["height"].get(),
    )

    if (
        gv.last_results.get_last_value() != result_from_model and result_from_model != "1"
