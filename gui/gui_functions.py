...
def popup_rectangle_window(button, x, y, width, height):
    ...
    canvas.create_rectangle(x.get(), y.get(), x.get(
    )+width.get(), y.get()+height.get(), outline="green", width=5)
    canvas.pack()
    button.configure(command=partial(destroy_rectangle_window,
...
def destroy_rectangle_window(window, button, x, y, width, height):
    ...
    button.configure(command=partial(
        popup_rectangle_window, button, x, y, width, height))
