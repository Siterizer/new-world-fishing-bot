...
from utils.config import config_dict, save_data
...
def change_repair_button_state(button):
    if config_dict["repairing"]["enable"].get() == 1:
        ...
        config_dict["repairing"]["enable"] = IntVar(value=0)
    else:
        button.configure(text="ON ")
        button.configure(bg="green")
        config_dict["repairing"]["enable"] = IntVar(value=1)


def change_bait_button_state(button):
    if config_dict["bait"]["enable"].get() == 1:
        button.configure(text="OFF")
        button.configure(bg="red")
        config_dict["bait"]["enable"] = IntVar(value=0)
    else:
        button.configure(text="ON ")
        button.configure(bg="green")
        config_dict["bait"]["enable"] = IntVar(value=1)
